          ...
          <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
            ...
            <div className="space-y-0.5 text-sm">
              ...
              <button
                ...
                className={`w-full flex justify-between items-center px-3 py-2 rounded-lg transition-colors text-left cursor-pointer font-medium
                  ${filterSelected === "root" ? "bg-gray-100 text-gray-900 font-bold" : "text-gray-600 hover:bg-gray-50"}`}
              >
                  ...
                  <div key={parent.id}>
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-1.5 px-3 py-2 rounded-lg transition-colors text-left cursor-pointer font-medium group
                        ${isParentSelected ? "bg-gray-100 text-gray-900 font-bold" : "text-gray-700 hover:bg-gray-50"}`}
                    >
                          ...
                          return (
                            <button
                              ...
                              className={`w-full flex items-center justify-between px-2.5 py-1.5 rounded-lg transition-colors text-left cursor-pointer text-[12px]
                                ${isChildSelected ? "bg-gray-100 text-gray-900 font-bold" : "text-gray-500 hover:bg-gray-50 font-medium"}`}
                            >
                              <span className="flex items-center gap-1.5 truncate">
                                <span className="w-1.5 h-1.5 rounded-full shrink-0 bg-gray-300" />
                                <span className="truncate">{child.name}</span>
                              </span>
                              <span className="text-[10px] px-1.5 py-0.5 rounded-full shrink-0 ml-1 bg-gray-100 text-gray-500">
          ...
          <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-gray-800">{countActive}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-500">Đã ẩn</span>
                <span className="font-bold text-gray-800">{countInactive}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-500">Danh mục gốc</span>
                <span className="font-bold text-gray-800">{rootCategories.length}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex-1 overflow-auto bg-gray-50/30 relative">
            <table className="w-full text-left border-collapse min-w-max">
              <thead className="sticky top-0 bg-gray-50 text-gray-600 z-10 text-sm">
                <tr>
                  <th className="py-3 px-6 font-bold border-b border-gray-200 w-12 text-center">STT</th>
                  <th className="py-3 px-6 font-bold border-b border-gray-200">Tên danh mục</th>
                  <th className="py-3 px-6 font-bold border-b border-gray-200">Danh mục cha</th>
                  <th className="py-3 px-6 font-bold border-b border-gray-200">Mô tả</th>
                  <th className="py-3 px-6 font-bold border-b border-gray-200 text-center w-20">Thứ tự</th>
                  <th className="py-3 px-6 font-bold border-b border-gray-200 text-center w-32">Trạng thái</th>
                  <th className="py-3 px-6 font-bold border-b border-gray-200 text-right w-28">Thao tác</th>
                </tr>
              </thead>
              <tbody className="text-sm text-gray-700">
                {isLoading ? (
                  ...
                  filteredCategories.map((cat, index) => (
                    <tr key={cat.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors bg-white">
                      ...
                      <td className="py-4 px-6 text-sm text-gray-700">
                        {cat.parent ? cat.parent : <span className="text-gray-400">Gốc</span>}
                      </td>
                      ...
                      <td className="py-4 px-6 text-center font-medium text-gray-700">{cat.order}</td>
