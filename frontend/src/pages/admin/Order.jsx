...
const mockOrders = [
  ...
  { id: "DH004", customerName: "Phạm Thị D", phone: "0933445566", address: "12 Trần Hưng Đạo, Q5, TP.HCM", date: "07/04/2026 09:00", total: 5990000, paymentMethod: "COD", paymentStatus: "Chưa thanh toán", orderStatus: "cancelled", items: [{ name: "Tai nghe Sony WH-1000XM5", variant: "Đen", qty: 1, price: 5990000 }] },
  { id: "DH005", customerName: "Hoàng Văn E", phone: "0966778899", address: "88 Lý Tự Trọng, Hải Châu, Đà Nẵng", date: "06/04/2026 11:20", total: 52000000, paymentMethod: "Chuyển khoản", paymentStatus: "Đã thanh toán", orderStatus: "completed", items: [{ name: "MacBook Air M3", variant: "Midnight 16GB", qty: 1, price: 52000000 }] },
];
...
const Order = () => {
  ...
  const [confirmModal, setConfirmModal] = useState({
    isOpen: false, actionType: null, orderId: null, newStatus: null, title: "", message: "",
  });
    ...
    <div className="flex w-full h-full bg-[#f0f2f5] overflow-hidden justify-center relative font-sans">
      <div className="flex w-full max-w-[1536px] h-full p-4 lg:p-6 gap-4 lg:gap-6">
        ...
        <div className="w-64 shrink-0 flex flex-col gap-4 overflow-y-auto hide-scrollbar pb-4 pr-1">
          ...
          {/* -- Lọc trạng thái đơn -- */}
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            <h3 className="font-bold text-gray-800 mb-3 text-sm">Trạng thái đơn hàng</h3>
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            <h3 className="font-bold text-gray-800 mb-3 text-sm">Trạng thái thanh toán</h3>
            <div className="space-y-0.5 text-sm">
              {[
                { value: "all",                label: "Tất cả" },
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-gray-500">Tổng đơn</span><span className="font-bold text-gray-800">{orders.length}</span></div>
              <div className="flex justify-between"><span className="text-gray-500">Chờ xác nhận</span><span className="font-bold text-gray-800">{countByStatus("pending")}</span></div>
              <div className="flex justify-between"><span className="text-gray-500">Đang giao</span><span className="font-bold text-gray-800">{countByStatus("processing")}</span></div>
              <div className="flex justify-between"><span className="text-gray-500">Hoàn thành</span><span className="font-bold text-gray-800">{countByStatus("completed")}</span></div>
              <div className="flex justify-between"><span className="text-gray-500">Đã hủy</span><span className="font-bold text-gray-800">{countByStatus("cancelled")}</span></div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden min-w-0">
          ...
          <div className="flex-1 overflow-auto bg-gray-50/30 relative">
            <table className="w-full text-left border-collapse min-w-max">
              <thead className="sticky top-0 bg-gray-50 text-gray-600 z-10 text-sm">
                <tr>
                  <th className="py-3 px-6 font-bold border-b border-gray-200">Mã ĐH</th>
                  <th className="py-3 px-6 font-bold border-b border-gray-200">Khách hàng</th>
                  <th className="py-3 px-6 font-bold border-b border-gray-200">Ngày đặt</th>
                  <th className="py-3 px-6 font-bold border-b border-gray-200 text-right">Tổng tiền</th>
                  <th className="py-3 px-6 font-bold border-b border-gray-200 text-center">Thanh toán</th>
                  <th className="py-3 px-6 font-bold border-b border-gray-200 text-center">Trạng thái</th>
                </tr>
              </thead>
                      ...
                      <React.Fragment key={order.id}>
                        ...
                        >
                          ...
                          <td className="py-4 px-6 text-center">
                            ...
                            <p className={`text-[10px] font-bold mt-1 uppercase ${order.paymentStatus === "Đã thanh toán" ? "text-green-600" : "text-orange-500"}`}>
                              {order.paymentStatus}
                            </p>
                          </td>
                          <td className="py-4 px-6 text-center">{renderStatusBadge(order.orderStatus)}</td>
                        </tr>
                              ...
                              <div className="flex flex-col xl:flex-row gap-6">
                                ...
                                <div className="w-full xl:w-48 shrink-0 flex flex-col gap-3">
                                  ...
                                  {order.orderStatus === "pending" && (
                                    <button
                                      onClick={() => setConfirmModal({ isOpen: true, actionType: "process", orderId: order.id, newStatus: "processing", title: "Xác nhận duyệt đơn", message: `Duyệt và chuyển đơn hàng ${order.id} sang trạng thái Đang giao?` })}
                                  ...
                                  {order.orderStatus === "processing" && (
                                    <button
                                      onClick={() => setConfirmModal({ isOpen: true, actionType: "complete", orderId: order.id, newStatus: "completed", title: "Xác nhận giao thành công", message: `Đơn hàng ${order.id} đã được giao thành công?` })}
                                  ...
                                  {(order.orderStatus === "pending" || order.orderStatus === "processing") && (
                                    <button
                                      onClick={() => setConfirmModal({ isOpen: true, actionType: "cancel", orderId: order.id, newStatus: "cancelled", title: "Cảnh báo hủy đơn", message: `Bạn có chắc chắn hủy đơn hàng ${order.id} không?` })}
          ...
          <div className="px-6 py-4 border-t border-gray-200 bg-white flex justify-between items-center text-sm text-gray-600 shrink-0">
            <span className="font-medium">
              Hiển thị <span className="font-semibold text-gray-800">{filteredOrders.length}</span> / {orders.length} đơn hàng
              {hasActiveFilter && (
                <button
