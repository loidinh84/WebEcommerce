...
const getMemberTier = (totalSpent) => {
  if (totalSpent >= 50000000) return { label: "Kim Cương", value: "diamond", cls: "bg-cyan-100 text-cyan-700 border border-cyan-200" };
  if (totalSpent >= 20000000) return { label: "Vàng",      value: "gold",    cls: "bg-yellow-100 text-yellow-700 border border-yellow-200" };
  if (totalSpent >= 5000000)  return { label: "Bạc",       value: "silver",  cls: "bg-gray-100 text-gray-600 border border-gray-300" };
  return                              { label: "Đồng",      value: "bronze",  cls: "bg-orange-100 text-orange-600 border border-orange-200" };
};
// ────────────────────────────────────────────────────────────────────────────
...
const mockCustomers = [
  {
    ...
    totalOrders: 5, totalSpent: 55000000, status: "active",
    recentOrders: [
  ...
  },
  {
    ...
    totalOrders: 3, totalSpent: 5500000, status: "banned",
    recentOrders: [
  ...
  {
    id: "KH004", name: "Phạm Thị D", email: "phamthid@gmail.com", phone: "0933445566",
    address: "12 Trần Hưng Đạo, Q5, TP.HCM", joinedDate: "10/03/2026",
    totalOrders: 2, totalSpent: 3200000, status: "active",
    recentOrders: [
...
const Customer = () => {
  ...
  const [confirmModal, setConfirmModal] = useState({
    isOpen: false, actionType: null, customerId: null, newStatus: null, title: "", message: "",
  });
  ...
  const fetchCustomers = async () => {
    try {
      ...
      setTimeout(() => { setCustomers(mockCustomers); setIsLoading(false); }, 400);
    } catch (error) {
  ...
  const clearAllFilters = () => {
    setFilterStatus("all"); setFilterTier("all");
  ...
  const filteredCustomers = customers.filter((c) => {
    ...
    // 2. Hạng thành viên
    if (filterTier !== "all" && getMemberTier(c.totalSpent).value !== filterTier) return false;
  ...
  const countByTier = (tierValue) =>
    tierValue === "all"
      ? customers.length
      : customers.filter((c) => getMemberTier(c.totalSpent).value === tierValue).length;
    ...
    <div className="flex w-full h-full bg-[#f0f2f5] overflow-hidden justify-center relative font-sans">
      <div className="flex w-full max-w-[1536px] h-full p-4 lg:p-6 gap-4 lg:gap-6">
        ...
        <div className="w-64 shrink-0 flex flex-col gap-4 overflow-y-auto hide-scrollbar pb-4 pr-1">
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            <h3 className="font-bold text-gray-800 mb-3 text-sm">Trạng thái tài khoản</h3>
          ...
          {/* -- Lọc hạng thành viên -- */}
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            <h3 className="font-bold text-gray-800 mb-3 text-sm">Hạng thành viên</h3>
            <div className="space-y-0.5 text-sm">
              {[
                { value: "all",     label: "Tất cả" },
                ...
                >
                  <div className="flex items-center gap-2">
                    <span>{item.label}</span>
                    {item.sub && <span className="text-[10px] text-gray-400 font-normal">{item.sub}</span>}
                  </div>
                  <span className="text-xs px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-500">
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            <h3 className="font-bold text-gray-800 mb-3 text-sm">Ngày tham gia</h3>
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-gray-500">Tổng khách hàng</span><span className="font-bold text-gray-800">{customers.length}</span></div>
              <div className="flex justify-between"><span className="text-gray-500">Đang hoạt động</span><span className="font-bold text-gray-800">{countByStatus("active")}</span></div>
              <div className="flex justify-between"><span className="text-gray-500">Đã khóa</span><span className="font-bold text-gray-800">{countByStatus("banned")}</span></div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden min-w-0">
            ...
            <table className="w-full text-left border-collapse min-w-max">
              <thead className="sticky top-0 bg-gray-50 text-gray-600 z-10 text-sm">
                <tr>
                  ...
                  <th className="py-3 px-6 font-bold border-b border-gray-200">Liên hệ</th>
                  <th className="py-3 px-6 font-bold border-b border-gray-200 text-center w-32">Hạng thành viên</th>
                  <th className="py-3 px-6 font-bold border-b border-gray-200 text-center w-28">Số đơn hàng</th>
                  <th className="py-3 px-6 font-bold border-b border-gray-200 text-right">Tổng chi tiêu</th>
                  <th className="py-3 px-6 font-bold border-b border-gray-200 text-center w-32">Trạng thái</th>
                </tr>
              </thead>
                      ...
                      <React.Fragment key={customer.id}>
                        ...
                        >
                          <td className="py-4 px-6">
                            ...
                            <p className="text-xs text-gray-400 mt-0.5">Mã: {customer.id} · Tham gia: {customer.joinedDate}</p>
                          </td>
                          <td className="py-4 px-6">
                          ...
                          <td className="py-4 px-6 text-center">
                            <span className={`px-2.5 py-1 rounded text-xs font-bold uppercase whitespace-nowrap ${tier.cls}`}>
                              {tier.label}
                            </span>
                          </td>
                          <td className="py-4 px-6 text-center font-bold text-gray-800">{customer.totalOrders}</td>
                          <td className="py-4 px-6 text-right font-bold text-gray-800">{formatPrice(customer.totalSpent)}</td>
                          <td className="py-4 px-6 text-center">
                              ...
                              <div className="flex flex-col xl:flex-row gap-6">
                                ...
                                <div className="flex-1 bg-white p-5 rounded-lg border border-gray-200 shadow-sm">
                                  ...
                                  <div className="space-y-3 text-sm">
                                    ...
                                    <div className="flex"><span className="text-gray-500 w-28 font-medium">Hạng:</span>
                                      <span className={`px-2 py-0.5 rounded text-xs font-bold uppercase ${tier.cls}`}>{tier.label}</span>
                                    </div>
          ...
          <div className="px-6 py-4 border-t border-gray-200 bg-white flex justify-between items-center text-sm text-gray-600 shrink-0">
            <span className="font-medium">
              Hiển thị <span className="font-semibold text-gray-800">{filteredCustomers.length}</span> / {customers.length} khách hàng
              {hasActiveFilter && (
                <button
