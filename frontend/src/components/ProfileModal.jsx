...
const ProfileModal = ({ isOpen, onClose }) => {
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`http://localhost:5000/api/user/update/${user.TENDANGNHAP}`, {
        method: "PUT",
      ...
      if (data.success) {
        toast.update(toastId, { render: "Cập nhật thông tin thành công!", type: "success", isLoading: false, autoClose: 2000 });
        setTempUser(user);
        setIsEditing(false);
      } else {
        toast.update(toastId, { render: "Lỗi từ server: " + data.message, type: "error", isLoading: false, autoClose: 3000 });
      }
    } catch (err) {
      toast.update(toastId, { render: "Đã lưu (Chế độ test UI)!", type: "success", isLoading: false, autoClose: 2000 });
      setTempUser(user);
      setIsEditing(false);
    }
  };
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      toast.update(toastId, { render: "Đã đổi mật khẩu (Chế độ test UI)!", type: "success", isLoading: false, autoClose: 2000 });
      setShowPasswordModal(false);
      setPasswordData({ oldPassword: "", newPassword: "", confirmPassword: "" });
    } catch (err) {
      toast.update(toastId, { render: "Lỗi hệ thống!", type: "error", isLoading: false, autoClose: 3000 });
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-6 font-inter">
      ...
      <div className="w-full max-w-7xl h-auto max-h-[95vh] bg-[#F8F9FD] rounded-[30px] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 relative">
        ...
        {isLoading ? (
          <div className="flex-1 flex items-center justify-center font-bold text-gray-500 py-32">Đang tải dữ liệu...</div>
        ) : (
          <div className="flex-1 overflow-y-auto p-6 lg:p-8 flex flex-col lg:flex-row gap-8">
            ...
            <div className="w-full lg:w-[380px] space-y-6 shrink-0">
              ...
              <div className="bg-white rounded-[25px] p-8 shadow-sm border border-gray-100 text-center relative">
                ...
                >
                  <div className="w-full h-full bg-gray-50 rounded-full flex items-center justify-center border-4 border-white shadow-md overflow-hidden transition-all group-hover:border-purple-100">
                    {user.HINHANH ? (
                      <img src={user.HINHANH} alt="Avatar" className="w-full h-full object-cover" />
                    ) : (
              ...
              <div className="bg-white rounded-[25px] p-8 shadow-sm border border-gray-100 space-y-6">
                <h4 className="font-bold text-gray-800 text-lg">Bảo mật tài khoản</h4>
            ...
            <div className="flex-1 bg-white rounded-[30px] p-8 lg:p-10 shadow-sm border border-gray-100 relative">
              <div className="flex justify-between items-center mb-8 pb-5 border-b border-gray-100">
                <div className="flex items-center gap-6">
                  ...
                  >
                    <span className="opacity-60 group-hover:opacity-100 group-hover:-translate-x-1 transition-all text-xl">&larr;</span>
                    <span className="font-bold text-gray-500 group-hover:text-[#5D5FEF] text-sm">Trở về</span>
                  </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-4">
                <InfoField label="Tên người dùng" name="TENDANGNHAP" value={user.TENDANGNHAP} isLocked={true} />
                <InfoField label="Họ và tên" name="TENNGUOIDUNG" value={user.TENNGUOIDUNG} isLocked={!isEditing} onChange={handleChange} />
                <InfoField label="Số điện thoại" name="SDT" value={user.SDT} isLocked={!isEditing} onChange={handleChange} />
                <InfoField label="Email" name="EMAIL" value={user.EMAIL} isLocked={!isEditing} onChange={handleChange} />

                <div className="md:col-span-2 grid grid-cols-2 gap-10">
                  <InfoField label="Ngày Sinh" name="NGAYSINH" value={user.NGAYSINH} type="date" isLocked={!isEditing} onChange={handleChange} />
                  <div className="space-y-2">
                    <label className="text-[11px] font-bold text-gray-400 uppercase ml-1 tracking-widest">Giới tính</label>
                    <div className="relative">
                      <select
                        name="GIOITINH" value={user.GIOITINH || "Nam"} onChange={handleChange} disabled={!isEditing}
                      ...
                      <span className="absolute right-4 top-1/2 -translate-y-1/2 opacity-30 pointer-events-none text-xs">▼</span>
                    </div>
                  </div>
                </div>

                <div className="md:col-span-2">
                  <InfoField label="Địa chỉ thường trú" name="DIACHI" value={user.DIACHI} isLocked={!isEditing} onChange={handleChange} />
                </div>
              </div>
              ...
              {isEditing && (
                <div className="flex gap-4 mt-6 pt-6 animate-in fade-in slide-in-from-bottom-2">
                  {/* Nút Lưu Thay Đổi */}
                  <button onClick={handleUpdateProfile} className="w-full py-4 bg-[#5D5FEF] text-white rounded-2xl font-black text-sm hover:shadow-lg transition-all active:scale-95 cursor-pointer">
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[250] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-lg rounded-[30px] p-10 shadow-2xl animate-in zoom-in duration-300">
            <div className="flex justify-between items-center mb-8 pb-4 border-b border-gray-100">
              <h3 className="text-xl font-bold text-gray-900">Đổi mật khẩu bảo mật</h3>
              {/* Nút X Đóng Modal Mật Khẩu */}
              <button onClick={() => setShowPasswordModal(false)} className="text-2xl opacity-40 hover:opacity-100 cursor-pointer">&times;</button>
            </div>
            <form onSubmit={handleChangePassword} className="space-y-5">
              <PasswordField label="Mật khẩu hiện tại" name="oldPassword" value={passwordData.oldPassword} onChange={handlePasswordChange} />
              <PasswordField label="Mật khẩu mới" name="newPassword" value={passwordData.newPassword} onChange={handlePasswordChange} />
              <PasswordField label="Xác nhận mật khẩu mới" name="confirmPassword" value={passwordData.confirmPassword} onChange={handlePasswordChange} />
              {/* Nút Cập Nhật Mật Khẩu */}
              <button type="submit" className="w-full py-4 bg-gray-900 text-white rounded-2xl font-black text-sm mt-6 hover:bg-black transition-all cursor-pointer">
...
function InfoField({ label, name, value, type = "text", isLocked = false, onChange, placeholder = "" }) {
  return (
    <div className="space-y-2">
      <label className="text-[11px] font-bold text-gray-400 uppercase ml-1 tracking-widest">{label}</label>
      <input
        type={type} name={name} value={value || ""} onChange={onChange} readOnly={isLocked} placeholder={placeholder}
        className={`w-full p-3.5 rounded-2xl border text-sm font-bold outline-none transition-all ${
          isLocked ? "bg-gray-100 border-transparent text-gray-500 cursor-not-allowed" : "bg-white border-gray-200 text-gray-800 focus:border-[#5D5FEF]"
        }`}
      />
    </div>
  );
...
function PasswordField({ label, name, value, onChange, placeholder }) {
  return (
    <div className="space-y-2">
      ...
      <input type="password" name={name} value={value} onChange={onChange} placeholder={placeholder} required className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl outline-none text-sm focus:border-gray-400" />
    </div>
  );
}
  ...
  return (
    <div className="flex justify-between items-center py-1">
      ...
      <div className="flex items-center gap-2">
        <div className={`w-1.5 h-1.5 rounded-full ${isDone ? "bg-green-500 animate-pulse" : "bg-red-400"}`}></div>
        <span className={`text-[10px] font-black uppercase tracking-wider ${isDone ? "text-green-600" : "text-red-500"}`}>
